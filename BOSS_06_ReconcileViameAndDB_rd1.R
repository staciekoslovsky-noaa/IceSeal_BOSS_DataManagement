...
sightings_db <- RPostgres::dbGetQuery(con, "SELECT hotspot_id, species, num_seals FROM surv_boss.hotspot WHERE hotspot_type = \'seal\'")
...
# Copy 2012 images to external HD
wd <- "//nmfs/akc-nmml/Polar_Imagery/Surveys_IceSeals/BOSS/2012"
setwd(wd)

dat <- data.frame(image = images2review[grepl("LOSI12",images2review) == TRUE], stringsAsFactors = FALSE)

dat$folder <- sapply(strsplit(dat$image, "_"), "[[", 2)
dat$camera_id <- sapply(strsplit(dat$image, "_"), "[[", 3)
...
dat$color <- paste(wd, "/Images/", dat$folder, "/", dat$loc, "/", dat$image, sep = "")

file.copy(dat$color, "//nmfs/akc-nmml/NMML_CHESS_Imagery/ImagesForDetectionDevelopment/BOSS/BOSS_color")
file.copy(dat$color, "E:/BOSS_color")

# Copy 2013 images to external HD
wd <- "//nmfs/akc-nmml/Polar_Imagery_2/Surveys_IceSeals_BOSS_2013"
setwd(wd)

dat <- data.frame(image = images2review[grepl("BOSS13",images2review) == TRUE], stringsAsFactors = FALSE)
