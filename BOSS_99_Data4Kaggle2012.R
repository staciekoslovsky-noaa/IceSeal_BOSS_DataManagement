...
files <- RPostgreSQL::dbGetQuery(con, "SELECT flight_id, camera_id, hotspot_id, thermal_image, slr_image, hotspot_dt FROM surv_boss.hotspot WHERE slr_image LIKE \'LOSI12%\' AND (hotspot_type = \'seal\' OR hotspot_type = \'other_animal\' OR hotspot_type = \'unknown\')")
dat <- files
...
write.csv(dat, "//nmfs/akc-nmml/NMML_CHESS_Imagery/ImagesForDetectionDevelopment/_BOSS_ImagesSelected4ImageDev.csv", row.names = FALSE)
